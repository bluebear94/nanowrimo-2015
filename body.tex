...
\q{
	...
	After constructing the card, it must be tested. This is as simple as casting the card and inspecting the results, multiple times in order to account for its chance of success as well as any variability in its ability. After testing its functionality, the card must be labeled with the necessary fields. Of course, if the same recipe is followed, then the card will have the same effects, and in that case only a single verification is necessary.
